...
betaT<-(pareto_pdf(T, b =1, loc=-0.55, scale=.7)+T^2 - 0.7)/3
betaT2<-(pareto_pdf(T2, b =1, loc=-0.55, scale=.7)+T2^2 - 0.7)/3
#Limits
n=length(betaT)-1
a = 0
b = 1
...
write.matrix(betaT,file="betaT_conv.csv", sep=",")
write.matrix(betaT2,file="betaT2_conv.csv", sep=",")
...
for (i in 1:n_sims) {
  ...
  cv<-cv.SOFR.con(Ngrid,y,X,T,constr = "convex")
  indmin<-which.min(cv)
  ###############################################
...
for (i in 1:n_sims) {
  ...
  write.matrix(X,file=paste("X_conv_", i,"_", N, ".csv", sep=""), sep= ",")
  ...
  write.matrix(y,file=paste("y_conv_", i,"_", N, ".csv", sep=""), sep= ",")
  ...
  out<-SOFR.con(N1,y,X,T,m,constr = "convex")
  # out.NULL<-SOFR.NULL(N1,y,X,T,m,constr = NULL)
  beta1t<-out$betaSOFR #constrained estimate
...
ggplot(df, aes(x = T)) +
  ...
  geom_line(aes(y = mean), color = "#5F3108", linetype = "solid", 
            linewidth = 1.2) +
  geom_line(aes(y = lower-0.0007), color = "#B99B81",linetype= "dashed", linewidth=1.2)+
  geom_line(aes(y = upper+0.001),color = "#B99B81", linetype="dashed", linewidth=1.2)+
  geom_hline(yintercept = 0, color = "#780000") +
  ylim(-0.05, 0.3) +
  theme_bw() +
  theme(axis.line = element_blank(),
