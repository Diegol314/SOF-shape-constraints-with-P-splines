...
#fitted_vals_matrix <- matrix(0, n_sims, m)
#
...
#  
#  
#  fit <- pfr(y ~ lf(X, argvals=T, k=20, bs="ps", m=2))
#  aa <- coef(fit, n=m)
#  val <- aa$value
#  plot(T, val, ylim=c(-0.05, 1))
#  lines(T, betaT)
#  
#  # Store fitted values in matrix
##   fitted_vals_matrix[i,] <- val
##   
#IMSE_uncons = IMSE_uncons + h*sum(t(betaT-val)^2%*%W)
## end_time= Sys.time()
## tiempo_unconstrained = end_time - start_time
#}
## Calculate mean and confidence intervals for each mesh point
##mean_fitted_vals <- apply(fitted_vals_matrix, 2, mean)
...
##IMSE_uncons = IMSE_uncons
#
#paste0('IMSE:', IMSE_uncons/n_sims*1000)
## paste0('Times:', tiempo_unconstrained)
```
...
#   
#   
#   fit <- pfr(y ~ lf(X, argvals=T, k=30, bs="ps", m=2))
#   aa <- coef(fit, n=m)
...
# mean_fitted_vals <- apply(fitted_vals_matrix, 2, mean)
# 
# IMSE_uncons[i] = h*sum(t(betaT-val)^2%*%W)
...
for (i in 1:n_sims) {
  ...
  cv<-cv.SOFR.con(Ngrid,y,X,T,constr = "inc")
  indmin<-which.min(cv)
  ###############################################
...
for (i in 1:n_sims) {
  ...
  out<-SOFR.con(N1,y,X,T,m,constr = "inc")
  #out.NULL<-SOFR.NULL(N1,y,X,T,m,constr = NULL)
  beta1t<-out$betaSOFR #constrained estimate
...
library(ggplot2)
library(gridExtra)
# Create a data fr,ame with the, values for ,T, betaT, and the results
df <- data.frame(T = T,
                 betaT = betaT,
...
# par(mfrow=c(1,2))
# 
#   plot(T,betaT, col='black', ylim=c(-0.05,0.75), type='l') #true coefficient function
#   lines(T,results_constrained$mean,col="black",lty=5) #unconstrained estimate
#   lines(T,results_constrained$lower,col="black",lty=3) #projection based C.I
#   lines(T,results_constrained$upper,col="black",lty=3)  #projection based C.I
#   abline(h=0, col='red')
#   
#   plot(T,betaT, col='black', ylim=c(-0.05,0.75), type='l') #true coefficient function
#   lines(T,results_unconstrained$mean,col="black",lty=5) #unconstrained estimate
#   lines(T,results_unconstrained$lower,col="black",lty=3) #projection based C.I
#   lines(T,results_unconstrained$upper,col="black",lty=3)  #projection based C.I
#   abline(h=0, col='red')
#   
```
