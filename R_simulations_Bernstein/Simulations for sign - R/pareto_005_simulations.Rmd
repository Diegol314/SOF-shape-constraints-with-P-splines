...
pareto_pdf <- function(x, b = 1, scale = 1, loc = 1){
  ...
  return(density)}
betaT<-pareto_pdf(T, b =0.04, loc=0, scale=1)
betaT2<-pareto_pdf(T2, b =0.04, loc=0, scale=1)
#Limits
n=length(betaT)-1
a = 0
b = 1
...
for (i in 1:n_sims) {
  ...
  write.matrix(X,file=paste("X_", i,"_", N, ".csv", sep=""), sep= ",")
  ...
  write.matrix(y,file=paste("y_", i,"_", N, ".csv", sep=""), sep= ",")
...
ggplot(df, aes(x = T)) +
  ...
  geom_hline(yintercept = 0, color = "#780000", size=0.5) +
  ylim(-0.2, 2) +
  theme_bw() +
